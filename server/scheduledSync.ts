...
export async function inventorySyncScheduledHandler(req: Request, res: Response) {
  try {
    ...
    const [ghadeer, projectDiscovery, sei, talay, talayBeach, yasRivaReserve, yasParkPlace] = await Promise.allSettled([
      ...
      refreshTalayBeachOfficialInventory({ trigger: "scheduled", triggeredBy: trigger }),
      refreshYasRivaReserveOfficialInventory({ trigger: "scheduled", triggeredBy: trigger }),
      refreshYasParkPlaceOfficialInventory({ trigger: "scheduled", triggeredBy: trigger }),
    ]);
    if ([ghadeer, projectDiscovery, sei, talay, talayBeach, yasRivaReserve, yasParkPlace].every(result => result.status === "rejected")) {
    ...
    const talayBeachResult = talayBeach.status === "fulfilled" ? talayBeach.value : null;
    const yasRivaReserveResult = yasRivaReserve.status === "fulfilled" ? yasRivaReserve.value : null;
    const yasParkPlaceResult = yasParkPlace.status === "fulfilled" ? yasParkPlace.value : null;
    ...
    const sourceCounts: Array<typeof zeroCounts> = [ghadeerCounts, seiCounts, talayCounts, talayBeachCounts, yasRivaReserveCounts, yasParkPlaceCounts];
    ...
    const rollups = [
      ...
      ...seiRollups,
      ...(talayResult?.rollups ?? []),
      ...(talayBeachResult?.rollups ?? []),
      ...(yasRivaReserveResult?.rollups ?? []),
      ...(yasParkPlaceResult?.rollups ?? []),
    ];
    ...
    if (shouldNotifyInventoryOwner(counts, newProjects) || newDirectoryProjects.length > 0) {
      notificationSent = await notifyOwner({
        ...
        content: [
          ...
          `Sources: World of Aldar directory; Al Ghadeer${ghadeerResult ? ` (${ghadeerResult.captureDate})` : " (refresh failed)"}; Sei Saadiyat${seiResult ? ` (${seiResult.captureDate})` : " (refresh failed)"}; Talay${talayResult ? ` (${talayResult.captureDate})` : " (refresh failed)"}; Talay Beach${talayBeachResult ? ` (${talayBeachResult.captureDate})` : " (refresh failed)"}; Yas Riva Reserve${yasRivaReserveResult ? ` (${yasRivaReserveResult.captureDate})` : " (refresh failed)"}; Yas Park Place${yasParkPlaceResult ? ` (${yasParkPlaceResult.captureDate})` : " (refresh failed)"}. Raw explorer labels are not NAS availability.`,
        ].join("\n"),
      });
    }
    ...
    return res.json({
      ok: true,
      runId: ghadeerResult?.runId ?? (seiResult && "runId" in seiResult ? seiResult.runId : null) ?? talayResult?.runId ?? talayBeachResult?.runId ?? null,
      counts,
      summary,
      ...
      sourceErrors: {
        ...
        talayBeach: talayBeach.status === "rejected" ? String(talayBeach.reason) : null,
        yasRivaReserve: yasRivaReserve.status === "rejected" ? String(yasRivaReserve.reason) : null,
        yasParkPlace: yasParkPlace.status === "rejected" ? String(yasParkPlace.reason) : null,
      },
      notificationSent,
      snapshotSource: "official World of Aldar directory plus Al Ghadeer, Sei Saadiyat, Talay, Talay Beach, Yas Riva Reserve, and Yas Park Place captures",
      captureDate: ghadeerResult?.captureDate ?? seiResult?.captureDate ?? talayResult?.captureDate ?? talayBeachResult?.captureDate ?? yasRivaReserveResult?.captureDate ?? yasParkPlaceResult?.captureDate ?? null,
    });
  } catch (err) {
