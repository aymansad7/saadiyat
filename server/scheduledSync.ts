...
export async function inventorySyncScheduledHandler(req: Request, res: Response) {
  try {
    ...
    const [ghadeer, projectDiscovery, sei, talay, talayBeach, yasRivaReserve] = await Promise.allSettled([
    ...
    if ([ghadeer, projectDiscovery, sei, talay, talayBeach, yasRivaReserve].every(result => result.status === "rejected")) {
    ...
    const sourceCounts: Array<typeof zeroCounts> = [ghadeerCounts, seiCounts, talayCounts, talayBeachCounts, yasRivaReserveCounts];
    ...
    return res.json({
      ...
      notificationSent,
      snapshotSource: "official World of Aldar directory plus Al Ghadeer, Sei Saadiyat, Talay, Talay Beach, and Yas Riva Reserve captures",
      captureDate: ghadeerResult?.captureDate ?? seiResult?.captureDate ?? talayResult?.captureDate ?? talayBeachResult?.captureDate ?? yasRivaReserveResult?.captureDate ?? null,
    });
  } catch (err) {
