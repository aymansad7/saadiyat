...
function describeSourceEvent(e: SourceEvent) {
  switch (e.eventType) {
    ...
    case "source_status_change": return <span className="flex items-center gap-1.5 flex-wrap"><span className="text-muted-foreground">Official source state</span><span>{e.fromSourceStatus ?? "Not published"}</span><ArrowRight className="h-3 w-3 text-muted-foreground" /><span className={isSold(e.toSourceStatus ?? null) ? "font-semibold text-rose-600 dark:text-rose-400" : "font-medium"}>{e.toSourceStatus ?? "Not published"}</span>{isSold(e.toSourceStatus ?? null) && <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-[0.6rem] uppercase tracking-wider text-rose-600 dark:text-rose-400">Sold · source</span>}</span>;
