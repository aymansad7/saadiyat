            ...
            <div className="max-h-[660px] divide-y divide-border overflow-y-auto">
              {displayedUnits.map(unit => (
                <div key={`${unit.dataset}:${unit.projectSlug}:${unit.buildingSlug}:${unit.unitName}`} className="group grid gap-3 px-5 py-4 transition-colors hover:bg-accent/35 sm:px-6 md:grid-cols-[minmax(0,1.45fr)_minmax(150px,0.8fr)_minmax(150px,0.7fr)_auto] md:items-center">
                  ...
                  <div className="flex flex-wrap items-center gap-2">{unit.href ? <Button asChild size="sm" className="gap-1.5 whitespace-nowrap"><Link href={unit.href}>Open card <ArrowUpRight className="h-3.5 w-3.5" /></Link></Button> : <span className="text-xs text-muted-foreground">Route unavailable</span>}<AldarOfficialUnitLink aldarLink={unit.aldarLink} unitName={unit.unitName} compact /></div>
                </div>
              ))}
            </div>
          )}
