    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="container py-8 space-y-8">
        ...
        <section className="rounded-xl border border-border bg-card overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-3 text-xs text-muted-foreground sm:px-6"><span>{salesInventory.data?.source === "last-synced" ? "Source: last stored Aldar sync snapshot" : "Source: bundled Aldar inventory baseline — run Sync to establish the stored snapshot"}</span><span className="shrink-0 font-mono">{displayedUnits.length.toLocaleString()} shown</span></div>
            ...
            <div className="max-h-[660px] divide-y divide-border overflow-y-auto">
              {displayedUnits.map(unit => (
                <div key={`${unit.dataset}:${unit.projectSlug}:${unit.buildingSlug}:${unit.unitName}`} className="group grid gap-3 px-5 py-4 transition-colors hover:bg-accent/35 sm:px-6 md:grid-cols-[minmax(0,1.45fr)_minmax(150px,0.8fr)_minmax(150px,0.7fr)_auto] md:items-center">
                  ...
                  <div className="flex items-center gap-2">{unit.href ? <Button asChild size="sm" className="gap-1.5 whitespace-nowrap"><Link href={unit.href}>Open card <ArrowUpRight className="h-3.5 w-3.5" /></Link></Button> : <span className="text-xs text-muted-foreground">Route unavailable</span>}{unit.aldarLink && <Button asChild variant="ghost" size="icon" className="h-8 w-8" aria-label={`Open Aldar source for ${unit.unitName}`}><a href={unit.aldarLink} target="_blank" rel="noreferrer"><ExternalLink className="h-3.5 w-3.5" /></a></Button>}</div>
                </div>
              ))}
            </div>
          )}
