  ...
  return (
    <div className="min-h-screen bg-muted/20"><SiteHeader />
      <main className="container space-y-6 py-8">
        ...
        <Card><CardHeader className="gap-3 border-b pb-4"><CardTitle className="flex items-center gap-2 text-base"><UserRound className="h-4 w-4" />Owner directory · {rows.length}</CardTitle><div className="relative max-w-md"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={q} onChange={e => setQ(e.target.value)} placeholder="Search owner name, phone, or email" /></div></CardHeader><CardContent className="p-0"><div className="divide-y">{owners.isLoading ? <p className="p-6 text-sm text-muted-foreground">Loading protected owner records…</p> : rows.length ? rows.map(owner => <div key={owner.id} className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-medium">{owner.displayName}</div><div className="mt-1 text-xs text-muted-foreground">{[owner.phone, owner.email, owner.sourceLabel].filter(Boolean).join(" · ") || "No contact source recorded"}</div><div className="mt-1 flex flex-wrap gap-1.5 text-xs text-muted-foreground"><span>{owner.linkCount} exact unit link{owner.linkCount === 1 ? "" : "s"}</span>{owner.unlinkedSourceCount ? <span className="rounded-full bg-slate-100 px-1.5 py-0.5">{owner.unlinkedSourceCount} unlinked source</span> : null}{owner.conflictSourceCount ? <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-amber-800">{owner.conflictSourceCount} source conflict</span> : null}</div></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => setSelectedOwnerId(owner.id)}>View file</Button><Button variant="outline" size="sm" onClick={() => { setSelectedOwnerId(owner.id); setLink(current => ({ ...current, ownerId: String(owner.id) })); }}>Select to link</Button></div></div>) : <p className="p-6 text-sm text-muted-foreground">No owner records yet. Create the first one above.</p>}</div></CardContent></Card>
      </main>
    </div>
  );
}
