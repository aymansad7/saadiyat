...
export default function AdminOwners() {
  ...
  const [ownerDirectoryLimit, setOwnerDirectoryLimit] = useState(150);
  const [reviewQueueLimit, setReviewQueueLimit] = useState(120);
  ...
  const ownerDirectoryInput = useMemo(() => ({ q: clean(q), limit: ownerDirectoryLimit }), [ownerDirectoryLimit, q]);
  const reviewQueueInput = useMemo(() => ({ limit: reviewQueueLimit }), [reviewQueueLimit]);
  const owners = trpc.propertyOwners.list.useQuery(ownerDirectoryInput, { enabled: user?.role === "master" });
  const ownerDetail = trpc.propertyOwners.detail.useQuery(
    { id: selectedOwnerId ?? 0 },
    { enabled: user?.role === "master" && selectedOwnerId !== null },
  );
  const reviewQueue = trpc.propertyOwners.reviewQueue.useQuery(reviewQueueInput, { enabled: user?.role === "master" });
    ...
    <div className="min-h-screen bg-muted/20"><SiteHeader />
      <main className="container space-y-6 py-8">
        <section className="flex flex-col justify-between gap-4 rounded-xl border bg-card p-5 shadow-sm md:flex-row md:items-end">
          <div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-primary"><ShieldCheck className="h-4 w-4" />Master Admin only</div><h1 className="text-2xl font-semibold">Owners · unified property register</h1><p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">This protected directory is the single view of all imported owners, exact linked units, projects, source records, and review status. The system never matches an owner by area, price, or name automatically.</p></div>
        ...
        <Card className="border-amber-200/80">
          ...
          <CardContent className="space-y-4 pt-5">
            ...
            {!reviewQueue.isLoading && (reviewQueue.data?.length ?? 0) === reviewQueueLimit ? <Button variant="outline" size="sm" onClick={() => setReviewQueueLimit(current => Math.min(current + 120, 2_000))}>Load more review rows</Button> : null}
          </CardContent>
        </Card>
        ...
        {selectedOwnerId !== null && (
          <Card className="border-primary/20">
            ...
            <CardContent className="grid gap-5 pt-5 lg:grid-cols-2">
              ...
              <div className="lg:col-span-2"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Imported source records</div><p className="mt-1 text-xs text-muted-foreground">Unlinked or conflicting source rows are retained here for Master review; select this owner above to add one or more exact unit links.</p><div className="mt-3 max-h-52 space-y-2 overflow-y-auto">{ownerDetail.isLoading ? <p className="text-sm text-muted-foreground">Loading source records…</p> : ownerDetail.data?.imports.length ? ownerDetail.data.imports.map(record => <div key={record.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-muted/20 px-3 py-2"><div><div className="font-mono text-sm">{record.sourceUnit ?? "No source unit"}</div><div className="mt-0.5 text-xs text-muted-foreground">{record.sourceFile} · {record.sourceSheet} row {record.sourceRow}{record.sourceProject ? ` · ${record.sourceProject}` : ""}</div></div><span className={record.matchStatus === "linked" ? "rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-800" : record.matchStatus === "conflict" ? "rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800" : "rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700"}>{record.matchStatus}{record.matchReason ? ` · ${record.matchReason.replaceAll("_", " ")}` : ""}</span></div>) : <p className="text-sm text-muted-foreground">No imported source rows on this owner record.</p>}</div></div>
            </CardContent>
          </Card>
        )}

        <Card><CardHeader className="gap-3 border-b pb-4"><CardTitle className="flex items-center gap-2 text-base"><UserRound className="h-4 w-4" />Unified owner directory · {rows.length}{rows.length === ownerDirectoryLimit ? "+" : ""}</CardTitle><div className="relative max-w-xl"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" value={q} onChange={e => { setQ(e.target.value); setOwnerDirectoryLimit(150); }} placeholder="Search owner, phone, email, project, unit, or source file" /></div><p className="text-xs text-muted-foreground">Every row shows its known exact properties and import provenance; select the owner for the complete protected file and documents. Results load in small batches so the complete directory remains responsive.</p></CardHeader><CardContent className="p-0"><div className="divide-y">{owners.isLoading ? <p className="p-6 text-sm text-muted-foreground">Loading protected owner records…</p> : rows.length ? rows.map(owner => <div key={owner.id} className="flex flex-col gap-3 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="font-medium">{owner.displayName}</div><div className="mt-1 text-xs text-muted-foreground">{[owner.phone, owner.email, owner.sourceLabel].filter(Boolean).join(" · ") || "No contact source recorded"}</div><div className="mt-2 flex flex-wrap gap-1.5 text-xs text-muted-foreground"><span>{owner.linkCount} exact unit link{owner.linkCount === 1 ? "" : "s"}</span><span>{owner.sourceRecordCount} source row{owner.sourceRecordCount === 1 ? "" : "s"}</span>{owner.unlinkedSourceCount ? <span className="rounded-full bg-slate-100 px-1.5 py-0.5">{owner.unlinkedSourceCount} unlinked source</span> : null}{owner.conflictSourceCount ? <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-amber-800">{owner.conflictSourceCount} source conflict</span> : null}</div></div><div className="flex shrink-0 gap-2"><Button variant="outline" size="sm" onClick={() => setSelectedOwnerId(owner.id)}>Open owner file</Button><Button variant="outline" size="sm" onClick={() => { setSelectedOwnerId(owner.id); setLink(current => ({ ...current, ownerId: String(owner.id) })); }}>Link property</Button></div></div><div className="grid gap-3 text-xs lg:grid-cols-3"><div className="rounded-md bg-muted/30 p-2.5"><div className="font-semibold uppercase tracking-wide text-muted-foreground">Exact properties</div><div className="mt-1.5 flex flex-wrap gap-1.5">{owner.links.length ? owner.links.slice(0, 8).map(linkItem => <Link key={linkItem.id} href={`/map?plot=${encodeURIComponent(linkItem.villaKey)}`} className="rounded border bg-background px-1.5 py-0.5 font-mono text-foreground hover:border-primary hover:text-primary">{linkItem.villaKey}</Link>) : <span className="text-muted-foreground">No exact link yet</span>}{owner.links.length > 8 ? <span className="rounded border bg-background px-1.5 py-0.5 text-muted-foreground">+{owner.links.length - 8} more</span> : null}</div></div><div className="rounded-md bg-muted/30 p-2.5"><div className="font-semibold uppercase tracking-wide text-muted-foreground">Projects & sources</div><div className="mt-1.5 leading-5 text-muted-foreground">{owner.sourceProjects.length ? owner.sourceProjects.join(" · ") : "Project not supplied"}{owner.sourceFiles.length ? <div className="mt-1 text-[11px]">{owner.sourceFiles.join(" · ")}</div> : null}</div></div><div className="rounded-md bg-muted/30 p-2.5"><div className="font-semibold uppercase tracking-wide text-muted-foreground">Import status</div><div className="mt-1.5 text-muted-foreground">{owner.linkedSourceCount} linked{owner.unlinkedSourceCount ? ` · ${owner.unlinkedSourceCount} unlinked` : ""}{owner.conflictSourceCount ? ` · ${owner.conflictSourceCount} conflict` : ""}</div>{owner.importRecords.length ? <div className="mt-1 text-[11px] text-muted-foreground">Latest: {owner.importRecords[0]?.sourceUnit ?? "no source unit"}</div> : null}</div></div></div>) : <p className="p-6 text-sm text-muted-foreground">No owner records yet. Create the first one above.</p>}</div>{!owners.isLoading && rows.length === ownerDirectoryLimit ? <div className="border-t p-4"><Button variant="outline" onClick={() => setOwnerDirectoryLimit(current => Math.min(current + 150, 5_000))}>Load more owners</Button></div> : null}</CardContent></Card>
      </main>
    </div>
  );
}
