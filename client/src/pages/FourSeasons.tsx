...
export default function FourSeasons() {
  ...
  const filtered = useMemo(() => {
    ...
    return FOUR_SEASONS_VILLAS.filter((villa) => {
      ...
      if (!isWithinAreaRange({ sqm: villa.plotAreaSqm, sqft: villa.plotAreaSqft }, areaUnit, areaMin, areaMax)) return false;
      if (!normalized) return true;
      return [villa.label, villa.villaType, villa.view ?? "", `${villa.bedrooms} br`]
        .some((value) => value.toLowerCase().includes(normalized))
        || matchesAreaQuery(normalized, { sqm: villa.plotAreaSqm, sqft: villa.plotAreaSqft })
        || matchesAreaQuery(normalized, { sqm: villa.builtUpAreaSqm, sqft: villa.builtUpAreaSqft });
    });
      ...
      <main className="container py-8 space-y-8">
        <section className="rounded-2xl border border-border bg-card p-5 sm:p-8 shadow-sm">
          ...
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <Stat label="Pending municipal records" value={String(FOUR_SEASONS_PENDING_SUMMARY.recordCount)} />
        ...
        <section className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
          <div className="p-4 sm:p-5 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Green means Available in the current sheet. Neutral villas are only master-plan references.</p>
            </div>
            ...
            <div className="relative min-w-[760px]">
              ...
              {FOUR_SEASONS_VILLAS.map((villa) => (
                <button
                  key={villa.villaKey}
                  type="button"
                  title={`${villa.label}${villa.status === "available" ? ` — ${formatPrice(villa.askingPriceAed)}` : ""}`}
                  aria-label={`Open ${villa.label}`}
                  onClick={() => selectFromPlan(villa)}
                  className={`absolute -translate-x-1/2 -translate-y-1/2 h-6 min-w-6 px-1 rounded-full border-2 border-white shadow-md text-[9px] font-bold transition-transform hover:scale-125 focus:outline-none focus:ring-2 focus:ring-primary ${
                    villa.status === "available" ? "bg-emerald-500 text-white" : "bg-slate-700/85 text-white"
                  } ${visibleNumbers.has(villa.villaNumber) ? "opacity-100" : "opacity-20"}`}
              ...
              <tbody>
                {filtered.map((villa) => (
                  <tr id={`villa-${villa.villaNumber}`} key={villa.villaKey} className="border-t border-border scroll-mt-28">
                    ...
                    <td className="px-4 py-3 text-right font-mono">{formatArea({ sqm: villa.plotAreaSqm, sqft: villa.plotAreaSqft }, areaUnit)}</td>
                    <td className="px-4 py-3 text-right font-mono">{formatArea({ sqm: villa.builtUpAreaSqm, sqft: villa.builtUpAreaSqft }, areaUnit)}</td>
                    <td className="px-4 py-3 text-right font-semibold">{formatPrice(villa.askingPriceAed)}</td>
                    <td className="px-4 py-3 text-right"><Link className="text-primary hover:underline" href={`/map?plot=${encodeURIComponent(villa.villaKey)}`}>Map</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <details className="rounded-xl border border-amber-300/60 bg-amber-50/60 dark:bg-amber-950/20 p-4">
          <summary className="cursor-pointer font-semibold">Municipal transactions pending land match ({FOUR_SEASONS_PENDING_SUMMARY.recordCount})</summary>
          ...
          <div className="mt-4 overflow-auto">
            <table className="w-full text-sm min-w-[760px]">
              ...
              <tbody>
                {FOUR_SEASONS_PENDING_TRANSACTIONS.map((tx) => (
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <div><span className="text-xs text-muted-foreground">Land</span><div className="font-semibold">{formatArea({ sqm: villa.plotAreaSqm, sqft: villa.plotAreaSqft }, areaUnit)}</div></div>
        <div><span className="text-xs text-muted-foreground">BUA</span><div className="font-semibold">{formatArea({ sqm: villa.builtUpAreaSqm, sqft: villa.builtUpAreaSqft }, areaUnit)}</div></div>
      ...
      {villa.historicalSpecSource && villa.status !== "available" && <p className="mt-3 text-[0.68rem] text-muted-foreground">Areas from historical specification reference only. No current availability implied.</p>}
